...
export async function POST(req) {
  try {
    ...
    const validCategories = ['Pants', 'Jeans', 'Shirts', 'Watches', 'Jackets', 'Kurtis', 'Sarees', 'Blouses', 'Dresses', 'Skirts', 'Tops', 'T-shirts', 'Shorts', 'Sweaters', 'Hoodies', 'Coats', 'Blazers', 'Trousers', 'Leggings', 'Activewear', 'Swimwear'];
    ...
    const validFrameTypes = ['Slim Fit', 'Regular Fit', 'Relaxed Fit', 'Tailored'];
    if (frameType && !validFrameTypes.includes(frameType)) {
      return NextResponse.json(
        { error: `Style must be one of: ${validFrameTypes.join(', ')}` },
        { status: 400 }
      );
    }

    const validGenders = ['Men'];
